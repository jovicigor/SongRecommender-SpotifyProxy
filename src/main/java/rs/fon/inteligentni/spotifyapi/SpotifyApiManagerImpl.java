...
public class SpotifyApiManagerImpl implements SpotifyApiManager {
	...
	private FullTrack convertToFullTrack(Track track) {
		...
		if (!simpleArtists.isEmpty()) {
			...
			for (int i = 0; i < simpleArtists.size(); i++) {
				...
				artistsIds += simpleArtists.get(i).getId();
			}
	...
	@Override
	public List<FullTrack> getTracksByTrackNames(List<String> names) {
		List<FullTrack> fullTracks = new ArrayList<FullTrack>();
		for (String name : names) {
			FullTrack fullTrack = getTrackByName(name);
			if (fullTrack != null) {
